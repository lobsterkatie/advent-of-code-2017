...
def get_picky_agreement_count(num_trials):
    """Count how many times the least significant 16 bits agree in
       num_trials, given that generator A only considers multiples of 4 and
       generator B only considers multiples of 8."""
    ...
    for trial_num in range(num_trials):
        ...
        #continue getting values from each generator until we have a "good"
        #one from each ("good" meaning a multiple of 4 or a multiple of 8)
        while True:
            #get a new value from generator A
            new_val_A = (prev_val_A * 16807) % 2147483647
            prev_val_A = new_val_A

            #if it's good, pause and give generator B a chance
            if new_val_A % 4 == 0:
                break
        while True:
            #get a new value from generator B
            new_val_B = (prev_val_B * 48271) % 2147483647
            prev_val_B = new_val_B

            #if it's good, pause and let the judge compare the numbers
            if new_val_B % 8 == 0:
                break

        #now that we have a number from each generatory, if their least
        #significant 16 bits match, add one to our count
        if bin(new_val_A)[-16:] == bin(new_val_B)[-16:]:
            count += 1
...
begin = datetime.now()
print get_picky_agreement_count(5000000)
print datetime.now() - begin
