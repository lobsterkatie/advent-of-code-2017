...
def find_largest_register_values(instructions):
    ...
    #keep track of the highest value seen at any point in the process
    max_ever = 0
    ...
    for instruction in instructions:
        ...
        if should_run:
            ...
            #update our max_ever if necessary
            if registers[register_to_change] > max_ever:
                max_ever = registers[register_to_change]

    #return the max register value at the end, as well as the max value ever
    end_max = max(registers.itervalues())
    return end_max, max_ever
...
with open("dec8.txt") as input_file:
    end_max, max_ever = find_largest_register_values(input_file.readlines())
    print "end max:", end_max
